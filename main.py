...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # 💡 সমাধান: প্রতি ১০টি ফাইল পরপর স্ট্যাটাস এডিট করা হবে যেন Telegram API Block না করে
        if status_msg_id and (count % 10 == 0 or count == 1):
            updated_text = (
...
async def done_merge(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'files' not in context.user_data or not context.user_data['files']:
        try:
            await context.bot.send_message(chat_id=chat_id, text="⚠️ No data loaded in buffer.", parse_mode="Markdown")
        except (Forbidden, TelegramError):
            pass
    ...
    try:
        ...
        if count == 0:
            await context.bot.send_message(chat_id=chat_id, text="⚠️ **PARSE FAILED**: No valid cards with CVV found.", parse_mode="Markdown")
        else:
            final_caption = (
                "🎯 **EXTRACTION COMPLETE**\n"
                "────────────────────────\n"
        ...
        try:
            await context.bot.send_message(chat_id=chat_id, text=f"❌ `SYSTEM ERROR`: {e}", parse_mode="Markdown")
        except (Forbidden, TelegramError):
            pass
    finally:
...
async def cancel_merge(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    gc.collect()
    try:
        await context.bot.send_message(chat_id=chat_id, text="🚫 **SESSION ABORTED & BUFFER PURGED**", parse_mode="Markdown")
    except (Forbidden, TelegramError):
        pass
